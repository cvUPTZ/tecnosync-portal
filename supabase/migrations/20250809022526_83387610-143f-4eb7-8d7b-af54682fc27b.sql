-- Fix academy creation functions search_path

CREATE OR REPLACE FUNCTION public.create_new_academy_with_user(academy_name text, academy_subdomain text, admin_full_name text, admin_email text, admin_password text, user_id uuid, modules_config jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.create_new_academy(academy_name text, academy_subdomain text, admin_full_name text, admin_email text, admin_password text, modules_config jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
