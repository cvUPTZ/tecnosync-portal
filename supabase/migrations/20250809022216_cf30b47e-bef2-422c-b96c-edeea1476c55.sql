-- CRITICAL SECURITY FIXES (Fixed casting error)
...
USING (
  EXISTS (
    ...
    WHERE user_id = auth.uid() 
    AND (
      role IN ('director'::app_role, 'coach'::app_role, 'platform_admin'::app_role)
    )
  )
);
...
USING (
  EXISTS (
    SELECT 1 FROM public.profiles 
    WHERE user_id = auth.uid() 
    AND role IN ('director'::app_role, 'coach'::app_role, 'platform_admin'::app_role)
  )
);
...
AS $function$
  SELECT EXISTS (
    ...
    FROM public.profiles
    WHERE user_id = auth.uid()
      AND role IN ('director'::app_role, 'comptabilite_chief'::app_role)
      AND is_active = true
  );
$function$;
...
USING (
  EXISTS (
    ...
    WHERE user_id = auth.uid() 
    AND (
      academy_id IS NOT NULL -- Users from same academy
      OR role = 'platform_admin'::app_role -- Platform admins
    )
  )
);
...
WITH CHECK (
  EXISTS (
    SELECT 1 FROM public.profiles 
    WHERE user_id = auth.uid() 
    AND role IN ('director'::app_role, 'coach'::app_role)
  )
);
